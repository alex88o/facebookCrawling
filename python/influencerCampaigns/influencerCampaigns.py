...
def alreadyExist(ID):
	...
	X = db.influencerPost.find({'id': ID})
	#print X.count()
	#print X.explain()
	return X.count()>0
	

def getReactionsSummary(postID):
	
	reac_path = postID + "?fields=reactions.type(LIKE).summary(total_count).limit(0).as(like),reactions.type(LOVE).summary(total_count).limit(0).as(love),reactions.type(WOW).summary(total_count).limit(0).as(wow),reactions.type(HAHA).summary(total_count).limit(0).as(haha),reactions.type(SAD).summary(total_count).limit(0).as(sad),reactions.type(ANGRY).summary(total_count).limit(0).as(angry)"

	params = urllib.urlencode({"access_token": ACCESS_TOKEN})
        host = "https://graph.facebook.com/v2.8/"

        url = "{host}{path}&{params}".format(host=host, path=reac_path, params=params)
	...
	try:
		...
		info = {}
		info['message'] = "Error occurred during getReactionsSummary() on postID:\t" + postID
...
def processResponse(pageId,res):
	...
	#	post['day'] = day		# day of analysis (progressive int)
		...
		if 'comments' in post:
			...
			# If we requested just the summary, skip the crawling of the entire list of comments and keep just the summary
			if 'summary' in post['comments']:
				print "Getting comments summary..."
				comm_summary = post['comments']['summary']
				post['comments'] = comm_summary
			else:
		...
		if 'reactions' in post:
			...
			print "Getting reactions summary..."
			reac_summary	=	getReactionsSummary(post['id'])
			post['reactions'] = reac_summary
	...
	#		shares			=	post['shares']
		...
		if len(global_buffer)==buffer_limit:
			...
			insertResult = db.influencerPost.insert_many(global_buffer)
...
def saveFanInfo(ID):
	...
	try:
		...
		if 'fan_count' in info:
			...
			insertResult = db.pageInfo.insert(info)
...
#	path = ID + "/posts?fields=id,message_tags,full_picture,shares,picture,type,permalink_url,message,created_time,caption,description,updated_time,targeting,feed_targeting,comments{from,comment_count,id,message,created_time,like_count,user_likes},reactions{type,name,id}&since=" + str(since) +	"&until=" + str(until)
	# Request without comment and reaction lists (too many requests at once)
	path = ID + "/posts?fields=id,message_tags,full_picture,shares,picture,type,permalink_url,message,created_time,caption,description,updated_time,targeting,feed_targeting,comments.summary(total_count).filter(stream)&since=" + str(since) +	"&until=" + str(until)
...
ACCESS_TOKEN = '1848071075471222|tY-TDMNfqEu5MZC54f22o5siRHM'
 ...
 # 	python influencerCampaigns.py ID,since,until
input = sys.argv[1] 
input_split = input.split(',')
pageID =input_split[0]
# One hour earlier since the server is in Italy, monitoring since one month before the campaign and one month after (seconds in a month: 3600*24*30)
since = int(input_split[1]) - 3600 - 3600*24*30
until = int(input_split[2]) - 3600 + 3600*24*30
...
print "Influencer Posts Analysis:\t starting crawling from page ID\t-\t" + str(pageID)
...
print "Influencer Posts Analysis:\tProcessed\t"+str(tot_posts)+ "\tposts for the user\t"+str(r)
print "(2 if the last response is empty)"
